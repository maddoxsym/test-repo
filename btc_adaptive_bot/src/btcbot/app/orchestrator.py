...
class Orchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        # Whether a setup can pay for itself. Runs before allocation, so the
        # allocator never explores a candidate the executor will refuse.
        self.eligibility = ActualTradeEligibility(
            self.config.actual_eligibility, self.config.risk
        )
        # The account's discovered taker rate, or None until OKX reports it.
        self._taker_fee_rate: float | None = None
        # Signals generated by strategies, of which only some are ever offered
        # as actual candidates. Counted separately so the dashboard can never
        # describe a shadow signal as a blocked demo order.
    ...
    async def bootstrap(self) -> Preconditions:
        ...
        # The account's real fee schedule. A cost model built on an assumed
        # rate is confidently wrong, so this is read from the exchange and only
        # falls back to configuration when OKX will not report it.
        if preconditions.demo_verified:
            await self._discover_fee_rates(capabilities.inst_id)
    ...
    async def _discover_fee_rates(self, inst_id: str) -> None:
        """Read the account's real taker rate; fall back loudly, never silently.

        The cost model decides which setups are worth trading, so an assumed
        fee rate is not a harmless default — at OKX's demo schedule the real
        rate is several times the config placeholder, and a model built on the
        placeholder would approve trades that lose money by construction.
        """
        if self.client is None:
            return
        try:
            fees = await self.client.get_fee_rates(inst_id)
        except (ApiError, TransportError) as exc:
            log.warning(
                "FEES",
                f"Could not read the account fee schedule ({exc}) — the eligibility "
                f"cost model falls back to the configured "
                f"{self.config.actual_eligibility.fallback_taker_fee_rate * 100:.4f}% "
                "taker rate, which may understate real costs.",
            )
            return
        if fees.taker <= 0:
            log.warning(
                "FEES",
                f"OKX reported a non-positive taker rate ({fees.taker}); keeping the "
                "configured fallback rather than modelling free trading.",
            )
            return
        self._taker_fee_rate = fees.taker
        log.info(
            "FEES",
            f"Account fee schedule: maker {fees.maker * 100:.4f}%, taker "
            f"{fees.taker * 100:.4f}% per side — a round trip costs "
            f"{fees.taker * 2 * 100:.4f}% before spread and slippage.",
        )

    def _trade_costs(self) -> TradeCosts:
        """Current round-trip cost estimate, using live spread where available."""
        config = self.config.actual_eligibility
        discovered = self._taker_fee_rate is not None
        return TradeCosts(
            taker_fee_rate=(
                self._taker_fee_rate if discovered else config.fallback_taker_fee_rate
            ),
            spread_bps=self.store.spread_bps if self.store else 0.0,
            slippage_bps=config.slippage_bps,
            source="exchange" if discovered else "config fallback",
        )
    ...
    def _pipeline_counters(self) -> dict[str, Any]:
        """Where every strategy signal ended up, as five distinct stages.

        These do not sum to each other and are not meant to: a signal can be
        refused by the decision layers *and* never assessed for eligibility.
        The point is that each number answers exactly one question, so
        ``blocked demo orders`` finally means orders that were actually
        blocked.
        """
        engine = self.decision.stats() if self.decision else {}
        eligibility = self.eligibility.stats
        executor = self.executor.stats() if self.executor else {}
        costs = self._trade_costs()
        return {
            # Every signal a strategy produced and that survived to be a
            # candidate for the decision layers.
            "signals_evaluated": self._signals_evaluated,
            # Refused by decision layers 1-7. Never reached eligibility.
            "decision_rejections": max(
                0, int(engine.get("evaluated", 0)) - int(engine.get("accepted", 0))
            ),
            # Assessed for actual eligibility, and found unable to pay for
            ...
            "eligibility_rejections": eligibility.shadow_only,
            "eligibility_reasons": eligibility.as_dict()["top_reasons"],
            # Reached the executor and were refused by layers 8-10.
            "final_execution_blocks": int(executor.get("rejected", 0)),
            # Actually sent to OKX.
            "actual_orders_sent": int(executor.get("submitted", 0)),
            "cost_model": {
                "taker_fee_pct": round(costs.taker_fee_rate * 100, 4),
                "round_trip_pct": round(costs.round_trip * 100, 4),
                "spread_bps": round(costs.spread_bps, 2),
                "slippage_bps": round(costs.slippage_bps, 2),
                "source": costs.source,
            },
        }

    def _journal_ineligible(self, signal: StrategySignal, verdict: EligibilityVerdict) -> None:
        """Record a shadow-only verdict in the same journal as every refusal.

        Layer **0**, deliberately: this runs before the order pipeline, so the
        setup never became an actual order candidate. Recording it as layer 8,
        9 or 10 would overstate what happened and inflate the blocked-order
        count with signals that were never orders.
        """
        if not (self.repos and self.experiment and self.experiment.state):
            return
        try:
            self.repos.rejected_signals.record(
                {
                    "experiment_id": self.experiment.state.experiment_id,
                    "ts_utc": iso(now_utc()),
                    "signal_id": None,
                    "setup_id": make_setup_id(
                        signal.strategy_id, signal.symbol, signal.timeframe,
            ...
            log.warning("ACTUAL ELIGIBILITY", f"Could not journal a shadow-only verdict: {exc}")
    ...
    async def _allocate_demo(self, candidates: list[tuple[str, StrategySignal]]) -> None:
        ...
        # --- actual-trade eligibility (pre-allocation) --------------------
        # Everything above decided whether the setup is *sound*. This decides
        # whether it can pay for itself. Setups that cannot stay shadow-only —
        # they are still trading in shadow research, they simply never become
        # actual candidates, so the allocator cannot explore into them.
        if self.config.actual_eligibility.enabled:
            costs = self._trade_costs()
            orderbook_valid = bool(context and context.features.orderbook_valid)
            assessed = surviving
            surviving, verdicts = self.eligibility.filter(
                assessed, costs=costs, orderbook_valid=orderbook_valid
            )
            # `filter` returns one verdict per input candidate, in order.
            for (_, signal), verdict in zip(assessed, verdicts, strict=True):
                if not verdict.eligible:
                    self._journal_ineligible(signal, verdict)
            if not surviving:
                log.info(
                    "ACTUAL ELIGIBILITY",
                    f"{len(verdicts)} candidate(s) remain shadow-only — none can cover "
                    f"{costs.describe()}",
                )
                return
    ...
    def dashboard_state(self) -> dict[str, Any]:
        ...
        return {
            ...
            # The signal funnel, stage by stage. Kept separate on purpose: a
            # strategy signal that never became an actual order candidate is
            # not a blocked demo order, and reporting it as one made the bot
            # look like it was constantly being refused when it was mostly
            # doing shadow research exactly as designed.
            "pipeline": self._pipeline_counters(),
            "research": {
