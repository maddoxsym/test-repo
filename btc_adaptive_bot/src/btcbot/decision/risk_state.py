...
from measured conditions — drawdown, daily loss, and the circuit-breaker
state. The label feeds the decision engine (entry gating), the leverage
engine (caps), and the dashboard.
...
* **PAUSED** — SAFE_MODE is active or the daily loss limit is hit; no new
  entries at all. Existing positions continue to be managed.
"""
...
class RiskStateTracker:
    ...
    def __init__(self, config: RiskConfig, breakers: CircuitBreakers) -> None:
        ...
        self._current = RiskStateSnapshot(NORMAL, "startup", 0.0, 0.0)
    ...
    def start_of_day(self, equity: float) -> None:
        """Record the UTC-day opening equity for the daily-loss limit."""
    ...
    def evaluate(self, *, equity: float, peak_equity: float) -> RiskStateSnapshot:
        ...
        if self.breakers.safe_mode.active:
            snapshot = RiskStateSnapshot(
                PAUSED, f"SAFE_MODE: {self.breakers.safe_mode.reason}", drawdown, daily_loss
            )
            ...
            snapshot = RiskStateSnapshot(
                REDUCED, f"drawdown {drawdown * 100:.1f}% (early de-risk)", drawdown, daily_loss
            )
        else:
            snapshot = RiskStateSnapshot(NORMAL, "conditions normal", drawdown, daily_loss)
